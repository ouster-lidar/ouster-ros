...
std::vector<diagnostic_msgs::msg::KeyValue> aggregate(
  const RingBuffer<MsgTimeInfo> time_info_buffer_)
{
  std::vector<diagnostic_msgs::msg::KeyValue> key_values;
  ...
  const auto time_diff_sum = std::accumulate(
    time_info_buffer_.begin(), time_info_buffer_.end(), rclcpp::Duration::from_seconds(0),
