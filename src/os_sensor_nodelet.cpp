...
std::string OusterSensor::get_sensor_hostname(ros::NodeHandle& nh) {
...
bool OusterSensor::update_config_and_metadata(sensor::client& cli) {
    sensor::sensor_config config;
...
void OusterSensor::save_metadata(ros::NodeHandle& nh) {
    ...
    if (write_text_to_file(meta_file, cached_metadata)) {
        ...
        NODELET_WARN_STREAM("Failed to write metadata to " << meta_file <<
...
void OusterSensor::create_get_config_service(ros::NodeHandle& nh) {
    get_config_srv =
...
void OusterSensor::create_set_config_service(ros::NodeHandle& nh) {
    set_config_srv =
        nh.advertiseService<SetConfig::Request, SetConfig::Response>(
            "set_config", [this](SetConfig::Request& request,
                ...
                try {
                    auto config_str = read_text_file(request.config_file);
                    config = sensor::parse_config(config_str);
                ...
                bool success = update_config_and_metadata(*sensor_client);
                response.config = cached_config;
                return success;
            });
...
std::pair<sensor::sensor_config, uint8_t> OusterSensor::create_sensor_config_rosparams(
    ros::NodeHandle& nh) {
...
void OusterSensor::populate_metadata_defaults(sensor::sensor_info& info,
...
void OusterSensor::connection_loop(sensor::client& cli, const sensor::packet_format& pf) {
